...
func (r *RemainingEntriesLogRepository) GetLastRemainingEntriesLogOnlyDecreaseByUserID(userID int) (model.RemainingEntriesLog, error) {
	...
	filter := bson.D{
		...
		{Key: "$expr", Value: bson.D{
			{Key: "$lt", Value: bson.A{"$new_entries", "$previous_entries"}},
		}},
	}
